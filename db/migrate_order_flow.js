...
//   3. Returned rentals move to an 'inspection' (Review) stage when the QR is
//      scanned, which also triggers a customer review request.
// ════════════════════════════════════════════════════════════════════════════
...
async function migrate() {
  try {
    ...
    //    and 'inspection' (returned rental being reviewed before re-listing).
    ...
    await db.query(`
      ALTER TABLE shoes ADD CONSTRAINT shoes_status_check CHECK (status IN (
        ...
        'listed','reserved','rented','inspection','sold',
        'rejected','returned_to_owner'
      ))
    `);
    console.log('  ✓ shoes status now includes reserved + inspection');

    // 3. Orders: add 'awaiting_payment' is NOT needed (we don't create unpaid orders),
    //    but add 'inspection' so a returned rental order can reflect the review stage,
    //    and add a paid_at column if missing.
    ...
    await db.query(`
      ALTER TABLE orders ADD CONSTRAINT orders_status_check CHECK (status IN (
        ...
        'active_rental','return_initiated','inspection','returned',
        'completed','cancelled','refunded'
      ))
    `);
    await db.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ`);
    console.log('  ✓ orders status includes inspection + paid_at present');
